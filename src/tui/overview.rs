...
fn ready(app: &App) -> (bool, bool, bool) {
    ...
    let nodes = !app.snap.store.nodes.is_empty()
        || native::array(app.doc(), "/outbounds")
            .iter()
            .any(|v| !["selector", "urltest", "direct", "block", "dns"].contains(&v["type"].as_str().unwrap_or("")));
...
pub fn key(app: &mut App, k: KeyEvent) {
    ...
    match k.code {
        K::Down | K::Char('j') => app.overview.group = (app.overview.group + 1).min(n.saturating_sub(1)),
...
pub fn hints(app: &App) -> Vec<(&'static str, &'static str)> {
    let (core, nodes, _) = ready(app);
    if !core || !nodes {
        return vec![("enter", "next step"), ("i", "import subscription"), (",", "config")];
    }
    vec![
        ("↑↓", "group"),
        ("enter", "choose proxy"),
...
pub fn draw(f: &mut Frame, area: Rect, app: &App) {
    ...
    if wide {
        ...
        let group_h = (groups(app).len() as u16 * 1 + 3).clamp(5, right.height / 2 + 2);
        ...
        draw_groups(f, g, app);
        if traffic.height >= 7 {
            let [t, a] = Layout::vertical([Constraint::Length(4), Constraint::Min(3)]).areas(traffic);
...
fn draw_status(f: &mut Frame, area: Rect, app: &App) {
    ...
    if !core || !nodes {
        let step = |done: bool, n: &str, label: &str, how: &str| {
            Line::from(vec![
                ...
                Span::styled(text::cell(label, 22), theme::s(if done { theme::dim() } else { theme::text() })),
        ...
        let lines = vec![
            ...
            Line::raw(""),
            step(core, "1", "Download sing-box core", if core { "" } else { ", → Core" }),
            step(nodes, "2", "Import a subscription", if nodes { "" } else { "i" }),
    ...
    let state = if running {
        vec![
            ...
            Span::styled(
                if s.started_at > 0 {
                    format!("  for {}", text::duration(crate::model::now().saturating_sub(s.started_at)))
                } else {
                    String::new()
                },
                theme::s(theme::dim()),
        ...
        vec![Span::styled("○ Stopped", theme::bold(theme::dim())), Span::styled("  s to start", theme::s(theme::faint()))]
    };
    let mode = match settings.route_mode.as_str() {
        "global" => vec![
            Span::styled("Global", theme::s(theme::text())),
            Span::styled(" → ", theme::s(theme::dim())),
            Span::styled(app.label(&settings.global_target), theme::s(theme::target(&settings.global_target))),
        ],
        "direct" => vec![Span::styled("Direct", theme::s(theme::text()))],
        _ => {
            let final_tag = app.doc().pointer("/route/final").and_then(|v| v.as_str()).unwrap_or("");
            vec![
                Span::styled("Rule", theme::s(theme::text())),
                Span::styled(format!("  {} rules, else ", native::array(app.doc(), "/route/rules").len()), theme::s(theme::dim())),
    ...
    let port = native::array(app.doc(), "/inbounds")
        ...
        .map(|i| format!("{}:{}", i["listen"].as_str().unwrap_or("127.0.0.1"), i["listen_port"]))
    ...
    let check_line = match check.state.as_str() {
        "passed" => vec![
            ...
            Span::styled(format!("  {} ago", text::ago(crate::model::now().saturating_sub(check.checked_at))), theme::s(theme::dim())),
        ],
        "failed" => vec![Span::styled(text::fit(&format!("✕ {}", check.detail), 34), theme::s(theme::bad()))],
        _ => vec![Span::styled(if running { "Not checked  v to check" } else { "—" }, theme::s(theme::faint()))],
    };
    let version = s.version.split_whitespace().last().unwrap_or("").trim_start_matches('v').to_string();
    let host = if s.ssh { format!(" · SSH {}", s.host) } else { String::new() };
    let lines = vec![
        ...
        kv("Capture", capture),
        kv("Check", check_line),
        kv("Core", vec![Span::styled(format!("sing-box {version}{host}"), theme::s(theme::dim()))]),
    ];
    f.render_widget(Paragraph::new(lines), area);
}
...
fn draw_traffic(f: &mut Frame, area: Rect, app: &App) {
    ...
    f.render_widget(Paragraph::new(vec![title("TRAFFIC", &right, area.width)]), head);
    ...
    if data.is_empty() {
        f.render_widget(
            Paragraph::new(Span::styled(
                if s.connected { "Waiting for samples…" } else { "Start the core to see traffic." },
                theme::s(theme::faint()),
            )),
            spark,
        );
    } else {
        f.render_widget(Sparkline::default().data(&data).style(theme::s(theme::accent())), spark);
    }
    if s.connected {
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(
                    format!("↓ {}  ↑ {}", text::bytes(s.status.downlink_total), text::bytes(s.status.uplink_total)),
                    theme::s(theme::text()),
                ),
                Span::styled(
                    format!(" · {} conns", app.history.total_live.max(s.status.connections_out as usize)),
                    theme::s(theme::dim()),
                ),
            ])),
            foot,
...
fn draw_groups(f: &mut Frame, area: Rect, app: &App) {
    let tags = groups(app);
    let mut lines = vec![title("PROXY GROUPS", if app.snap.connected { "live" } else { "defaults" }, area.width)];
    if tags.is_empty() {
        lines.push(Line::styled("  No groups yet · 2 → Groups → n", theme::s(theme::faint())));
    }
    ...
    for (i, tag) in tags.iter().enumerate().skip(start).take(height) {
        let v = native::array(app.doc(), "/outbounds").iter().find(|v| native::tag(v) == tag).cloned().unwrap_or_default();
        ...
        lines.push(modal::row(
            ...
            vec![
                Span::styled(text::cell(&app.label(tag), name_w), theme::bold(theme::text())),
                Span::styled(text::cell(kind, 8), theme::s(theme::dim())),
                Span::styled(text::cell(&app.label(&current), member_w), theme::s(theme::target(&current))),
                Span::styled(text::right(&proxies::delay_text(delay), 8), theme::s(proxies::delay_color(delay))),
            ],
        ));
    }
    f.render_widget(Paragraph::new(lines), area);
...
fn draw_apps(f: &mut Frame, area: Rect, app: &App) {
    ...
    if apps.is_empty() {
        lines.push(Line::styled(
            if app.snap.connected { "  No traffic observed yet." } else { "  —" },
            theme::s(theme::faint()),
        ));
    }
    ...
    for a in apps.iter().take(area.height.saturating_sub(1) as usize) {
        ...
        let name = if a.name.is_empty() { "Unknown app".to_string() } else { a.name.clone() };
        let top_target = a.targets.iter().max_by_key(|(_, n)| **n).map(|(t, _)| t.clone()).unwrap_or_default();
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled("▯".repeat(bar_w - filled.min(bar_w)), theme::s(theme::faint())),
            Span::styled(text::right(&text::bytes(total), 10), theme::s(theme::dim())),
            Span::styled(format!("  {}", text::fit(&app.label(&top_target), 14)), theme::s(theme::target(&top_target))),
        ]));
    }
    f.render_widget(Paragraph::new(lines), area);
}
